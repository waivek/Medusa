...
class MapEditor:
    def __init__(self):
        ...
        for i in range(len(self.ents1_spr)):
            self.ents1_spr[i].set_location((i*32,0))

        for i in range(len(self.ents2_spr)):
            self.ents2_spr[i].set_location((i*32,32))

        self.key_spr = []
        for i in range(KeyEnum.NUM.value):
            self.key_spr.append(Sprite(KeySprites[i]))
            self.key_spr[len(self.key_spr) - 1].set_location((i*32,64))

        self.lock_spr = []
        for i in range(KeyEnum.NUM.value):
            self.lock_spr.append(Sprite(LockSprites[i]))
            self.lock_spr[len(self.lock_spr) - 1].set_location((i * 32, 96))
